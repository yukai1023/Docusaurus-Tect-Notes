...
- `<template>`：用於標記一個不會在頁面加載時呈現，但可以在 JavaScript 運行時使用的 HTML 內容。
...
- `<bdi>` (Bidirectional Isolation)：用於隔離一段文本，以便正確呈現混合方向的文字，特別是在雙向語言環境中（如阿拉伯語和希伯來語）。
- `<bdo>` (Bidirectional Override)：允許手動指定文本方向，無論其周圍內容的方向如何。
